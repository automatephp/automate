...

{
    ...
    // check version
    check(
        'You have a supported version of PHP (>= 5.5.9).',
        'You need PHP 5.5.9 or greater.',
        function () {
    ...
    $manifest = file_get_contents('https://automatephp.github.io/automate/manifest.json');
    ...
    echo " - Downloading Automate v", Dumper::toString($current->version), "...$n";

    file_put_contents($current->name, file_get_contents($current->url));

    echo " - Checking file checksum...$n";

    if ($current->sha1 !== sha1_file($current->name)) {
        unlink($current->name);
